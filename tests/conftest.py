...
# The plugin at ``tests/_isolate_plugin.py`` runs each test in a freshly
# spawned Python subprocess. It is registered via ``addopts = "-p
# tests._isolate_plugin"`` in pyproject.toml — loading there (instead of via
# ``pytest_plugins`` here) sidesteps a PytestAssertRewriteWarning that would
# fire on every run. The plugin replaces the historic _reset_module_state
# autouse fixture (manual state clearing) with true process isolation. Works
# on Linux, macOS, and Windows (uses ``multiprocessing.spawn``, not POSIX
# fork).
#
# See ``tests/_isolate_plugin.py`` for the full design notes.
...
# ── Module-level state reset — replaced by subprocess isolation ────────────
#
# Each test runs in a freshly-spawned subprocess via
# ``tests/_isolate_plugin.py``, so module-level dicts / sets / ContextVars
# from one test cannot leak into the next. No manual per-module clearing
# needed.
#
